...
void sparse_multiply(
    ...
    // TODO
    int nnz = 0;
    // Build the CSR representation of A
    for (int i = 0; i < rows; i++)
    {
        row_ptrs[i] = nnz; // start of row i in values and col_indices
    ...
    row_ptrs[rows] = nnz; // marks end of CSR data
    *out_nnz = nnz; 

    // Compute y = A * x using the CSR representation
    for (int i = 0; i < rows; i++)
    {
        double sum = 0;
